...
- 2026-08-01: 最終レビュー（archive 後の事後検証）で5件の指摘。**2件だけ反映し3件は見送った**。判断基準は「そのスクリプトが今後も走るか」。①`check-setup.sh` は今後も新環境セットアップと「動かない」ときの切り分けで使うため、ファイルの有無（`-s`）だけでなく**必須キーが非空か・`REPLACE_ME` が残っていないか**まで見るようにした（雛形コピーのまま ✅ を出す経路と、`GARBAGE=x` だけの非空ファイル7件を ✅ と誤報告する経路を潰した）②`migrate-local-secrets.sh` の mv/cp の成否を判定し、コピーは一時ファイル→サイズ検証→rename に変更、失敗件数が1以上なら非ゼロ終了にした（部分コピーで「✅ 揃っています」と誤報告する経路を潰した）
- 2026-08-01: 上記で**見送った3件と理由**。①secret の特殊文字（`"` `\` `&`）対応 → 実際の値は英数字35文字で検証済み、かつ移行スクリプトは2台とも完了済みで再実行されない。なお同じ弱点は毎回走る `generate-projects.sh` にもあるので、secret をローテートするときに思い出すこと ②旧 README の「全55コミット」→ レビュアー自身が「統合対象の c30a3d4 までの55件は確実に取り込まれている」と確認済み。56件目は README 自身で取り込まれていないのが正しい。archive 解除してまで直す価値なし ③集計ラベル → 「処理済み」に変えるついでに直したが、単独では対応しない程度のもの
