...
> **注意**: このドキュメントは 2026-08-01 のモノレポ統合より前に書かれており、パス表記は当時のものです。読み替えは [docs/README.md](./README.md) を参照。
...
- 日付計算を UTC → JST (UTC+9) に修正
- `toJST()` ヘルパーで +9 時間オフセットを適用
- JST 境界テスト追加（UTC 23:00 → JST 翌日、UTC 00:30 → JST 同日）

### 3-8. テスト ✅

- date ユーティリティのユニットテスト (11件)
...
### 4-6. UI デザイン（DayTask 風） ✅

- クリーム黄色の角丸カード背景（固定枠、内部スクロール）
- カードにシャドウ付与
- ベージュ系の外側背景色
- カスタムチェックボックス（角丸スクアークル、ゴールド色、暗いチェックマーク）
- 文字色を薄い黒（darkGray）、フォントサイズ 15pt
- スクロールバー非表示
- 空状態は「No tasks」のミニマルデザイン

### 4-7. 動作確認 ✅

- [x] シミュレータでビルド・起動
- [x] タスクの追加・完了・削除・並べ替えを実際に操作して確認
- [x] 日付ナビゲーションの動作確認
- [x] 実機（iPhone）にインストールして確認
...
- XcodeGen で `TodoWidgetExtension` ターゲット追加
- 大サイズ（systemLarge）のウィジェット
- 今日のタスク最大12件を表示

### 6-2. Timeline Provider 実装 ✅

- 15分間隔で API からタスクを取得・更新
- タスク変更時に `WidgetCenter.shared.reloadAllTimelines()` で即時更新
- エラー時は赤字でメッセージ表示

### 6-3. UI デザイン ✅

- アプリと統一したカスタムチェックボックス（ゴールド色）
- クリーム黄色の背景色
- 文字色・フォントサイズをアプリに合わせて調整

### 6-4. 動作確認 ✅

- シミュレータ・実機でウィジェットの表示確認済み
